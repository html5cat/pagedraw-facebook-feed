# Generated by https://pagedraw.io/pages/8463
React = require 'react'
render = ->
    <div className="projectitem">
        <style dangerouslySetInnerHTML={__html: """
            @import url('https://fonts.googleapis.com/css?family=Roboto:');
            
            .projectitem {
                display: flex;
                flex-grow: 1;
            }
            
            .projectitem-b-5 {
                display: flex;
                flex-direction: column;
                padding-bottom: 1px;
                flex-grow: 1;
            }
            
            .projectitem-0-0-0 {
                display: flex;
            }
            
            .projectitem-rectangle_1 {
                display: flex;
                flex-direction: column;
                flex-grow: 1;
                flex-basis: 0px;
                min-height: fit-content;
                padding-top: 9px;
                padding-bottom: 11px;
                border-radius: 5px;
                background: rgb(99, 60, 252);
            }
            
            .projectitem-0-0-0-0-0 {
                display: flex;
                padding-left: 11px;
                padding-right: 11px;
            }
            
            .projectitem-pied_piper_-1 {
                flex-grow: 1;
                flex-basis: 0px;
                font-family: "Roboto", sans-serif;
                color: rgb(255, 255, 255);
                font-size: 14px;
                line-height: normal;
                letter-spacing: 0px;
                font-weight: bold;
                font-style: normal;
                text-decoration: none;
                text-align: left;
                word-wrap: break-word;
                overflow: hidden;
                text-overflow: ellipsis;
                white-space: nowrap;
            }
            
            .projectitem-a-7 {
                display: flex;
                flex-direction: column;
                padding-top: 10px;
                padding-bottom: 11px;
                background: #FFFFFF;
                flex-grow: 1;
            }
            
            .projectitem-1-0-0 {
                display: flex;
                padding-left: 10px;
                padding-right: 23px;
            }
            
            .projectitem-pied_piper_-6 {
                width: 231px;
                flex-shrink: 0;
                cursor: pointer;
                font-family: "Roboto", sans-serif;
                color: #000000;
                font-size: 14px;
                line-height: normal;
                letter-spacing: 0px;
                font-weight: bold;
                font-style: normal;
                text-decoration: none;
                text-align: left;
                word-wrap: break-word;
                overflow: hidden;
                text-overflow: ellipsis;
                white-space: nowrap;
            }
            
            * {
                box-sizing: border-box;
            }
            
            body {
                margin: 0;
            }
            
            button:hover {
                cursor: pointer;
            }
            
            a {
                text-decoration: none;
                color: inherit;
            }
            
            .pd-onhover-parent >.pd-onhover {
                display: none;
            }
            
            .pd-onhover-parent:hover > * {
                display: none;
            }
            
            .pd-onhover-parent:hover > .pd-onhover {
                display: flex;
            }
            
            .pd-onactive-parent > .pd-onactive {
                display: none;
            }
            
            .pd-onactive-parent:active > * {
                display: none;
            }
            
            .pd-onactive-parent:active > .pd-onactive {
                display: flex;
            }
            
            .pd-onactive-parent.pd-onhover-parent:active > .pd-onhover {
                display: none;
            }
        """} /> 
        { if ((if this.props.selected then 'B' else 'A') == "B")
            <div className="projectitem-b-5">
                <div className="projectitem-0-0-0">
                    <div className="projectitem-rectangle_1">
                        <div className="projectitem-0-0-0-0-0">
                            <div className="projectitem-pied_piper_-1">
                                { this.props.name }
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        }
        { if ((if this.props.selected then 'B' else 'A') == "A")
            <div className="projectitem-a-7">
                <div className="projectitem-1-0-0">
                    <div onClick={@props.onClick} className="projectitem-pied_piper_-6">
                        { this.props.name }
                    </div>
                </div>
            </div>
        }
    </div>

module.exports = (props) -> render.apply({props})